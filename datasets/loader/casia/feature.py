...
class MPFDecoder:
    ...
    def _head(self, mpf):
        ...
        header_size = struct.unpack('i', mpf.read(4))[0]
        # print('header_size',header_size)
        # 文件保存的形式，如 “MPF”
        self.code_format = mpf.read(8).decode('ascii').rstrip('\x00')
        # print('code format',self.code_format)
        # 文本说明
        ...
        # print('Code Type  ',self.code_type)
        # 编码长度
        ...
        # 样本数
        self.nrows = struct.unpack('i', mpf.read(4))[0]
        # print('nrows      ', self.nrows)
        # 特征向量的维数长度
        self.ndims = struct.unpack('i', mpf.read(4))[0]
    ...
    def _decode(self, mpf):
        '''解码 MPF 数据'''
        self._head(mpf)
        dataset = [[self._get_label(mpf), self._get_feature(mpf)] for _ in range(self.nrows)]
        dataset = DataFrame.from_records(dataset)
        self.dataset = DataFrame(data=np.column_stack(dataset[1]).T, index=dataset[0])
        ...
        for name in Z.namelist():
            if name.endswith('.mpf'):
                with Z.open(name) as mpf:
                    # print("Current MPF-> ", name)
                    decoder = MPFDecoder(mpf)
                    mb[name] = {"text": decoder.text,'dataset': decoder.dataset}
        return mb
